// Dixon Coles model, including temporal decay, no sum-to-0 constraint
// Uses the exact outcome for the likelihood function
...
functions{
  ...
  //custom pmf including correction and temporal decay.
  real dixon_coles_lpmf(int[] goals, int time, real rho, real decay, real log_rate_home, real log_rate_away){
    ...
    return exp(-decay * time) * (logp_home + logp_away + log_tau);
  }
}
...
data{
  ...
  // how many days has it been since the game
  int time[n_matches];
...
model{
  ...
  for (m in 1:n_matches){
    ...
    target += dixon_coles_lpmf(goals | time[m], rho, decay, log_rate_home[m], log_rate_away[m]);
  }
